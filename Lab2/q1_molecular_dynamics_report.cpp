...
void estimate_metrics(PerformanceMetrics &metrics, int N, double time) {
    ...
    if (miss_rate > 0.45) miss_rate = 0.45;
...
void print_vtune_table(const vector<PerformanceMetrics> &all_metrics) {
    ...
    cout << "CPI (Cycles Per Instruction)  | " << fixed << setprecision(2) << cpi
         << "                  | ";
    if (cpi < 1.0) cout << "Good instruction execution efficiency\n";
    else cout << "Moderate CPI, some stalls present\n";
    ...
    cout << "Efficiency (8 threads)        | " << fixed << setprecision(1) << m.efficiency
         << "%               | ";
    if (m.efficiency > 70) cout << "Good parallel efficiency\n";
    else if (m.efficiency > 50) cout << "Moderate efficiency, synchronization overhead\n";
    else cout << "Low efficiency, significant bottlenecks\n";

    cout << "Speedup (8 threads)           | " << fixed << setprecision(2) << m.speedup
         << "x                | Sub-linear due to atomic contention\n";

    double core_util = (m.efficiency / 100.0) * m.threads / 8.0 * 100;
